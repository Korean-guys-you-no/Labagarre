...
namespace JoyConUnity
{
	...
	public class JoyCon
	{
		...
		{
			return GetVector(new Vector3(j_b.x, i_b.x, k_b.x), -(new Vector3(j_b.z, i_b.z, k_b.z)));
		}

		private Quaternion GetVector(Vector3 forward, Vector3 up)
		{
		...
		private int ReceiveRaw()
		{
			...
			if (ret > 0)
			{
				lock (reports)
				{
					...
					rawUp = (raw_buf[3 + (isLeft ? 2 : 0)] & (isLeft ? 0x02 : 0x02)) != 0;
		...
		{
			string str = "JoyCon " + (isLeft ? "Left" : "Right") + Environment.NewLine;
			...
			str += "Stick Axis: " + GetStick().x + " - " + GetStick().y + Environment.NewLine;
		...
		{
			gyr_r[0] = (short)(reportBuf[19 + n * 12] | ((reportBuf[20 + n * 12] << 8) & 0xff00));
			gyr_r[1] = (short)(reportBuf[21 + n * 12] | ((reportBuf[22 + n * 12] << 8) & 0xff00));
			gyr_r[2] = (short)(reportBuf[23 + n * 12] | ((reportBuf[24 + n * 12] << 8) & 0xff00));
			acc_r[0] = (short)(reportBuf[13 + n * 12] | ((reportBuf[14 + n * 12] << 8) & 0xff00));
			acc_r[1] = (short)(reportBuf[15 + n * 12] | ((reportBuf[16 + n * 12] << 8) & 0xff00));
			acc_r[2] = (short)(reportBuf[17 + n * 12] | ((reportBuf[18 + n * 12] << 8) & 0xff00));
			for (var i = 0; i < 3; ++i)
			{
				...
				//	max[i] = acc_g[i];
				acc_g.SetAxisValue(i, acc_r[i] * 0.00025f);
				gyr_g.SetAxisValue(i, (gyr_r[i] - gyr_neutral[i]) * 0.00122187695f);
				if (Math.Abs(acc_g.GetAxisValue(i)) > Math.Abs(max[i]))
					max[i] = acc_g.GetAxisValue(i);
			}
		}
		...
		private int ProcessIMU(byte[] report_buf)
		{
			...
			for (int n = 0; n < 3; ++n)
			{
				...
				else
				{
					...
					err = (float)(Vector3.Dot(i_b, j_b) * 0.5);
